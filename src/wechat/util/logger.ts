...
/**
 * Plugin logger — writes JSON lines to a log file:
 *   <logDir>/dsh-plugin-wechat-YYYY-MM-DD.log
 *
 * NOTE: the original resolved `resolvePreferredOpenClawTmpDir()` from
 * `openclaw/plugin-sdk/infra-runtime`; here the log dir is
 * `DSH_WECHAT_LOG_DIR` or `<stateDir>/logs`.
 */

const MAIN_LOG_DIR =
  process.env.DSH_WECHAT_LOG_DIR?.trim() || path.join(resolveStateDir(), "logs");
const SUBSYSTEM = "wechat";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["dsh-plugin-wechat"];
...
function resolveMinLevel(): number {
  const env = process.env.DSH_WECHAT_LOG_LEVEL?.toUpperCase();
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `dsh-plugin-wechat-${dateKey}.log`);
}
