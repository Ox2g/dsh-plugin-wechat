...
/** Channel identifier used for OriginatingChannel / Provider / message sid prefix. */
export const CHANNEL_ID = "weixin";
 ...
 * Called once during account start to survive restarts.
 */
...
function generateMessageSid(): string {
  return generateId(CHANNEL_ID);
}

/**
 * Inbound context passed to the downstream message pipeline.
 *
 * NOTE: the original matched OpenClaw's `MsgContext` shape and included fields
 * consumed by the OpenClaw channel runtime (`SessionKey`, `CommandBody`,
 * `CommandAuthorized`). Those are kept here so the DSH adapter can map them;
 * they have no OpenClaw dependency at the type level.
 */
export type WeixinMsgContext = {
  ...
  ChatType: "direct";
  /** Set by the adapter after routing so the reply uses the correct session. */
  SessionKey?: string;
...
/**
 * Convert a WeixinMessage from getUpdates to the inbound context for the message pipeline.
