...
function isOwnPackageJson(parsed: PackageJson): boolean {
  ...
  return (
    ...
    (parsed.name.includes("dsh-plugin-wechat") || parsed.name.includes("openclaw-weixin"))
  );
}
...
/**
 * iLink-App-Id: read from package.json top-level `ilink_appid` field, falling
 * back to the ilink bot protocol default `"bot"`.
 */
 ...
 */
const DEFAULT_BOT_AGENT = "dsh-plugin-wechat";
...
export async function apiGetFetch(params: {
  ...
  try {
    ...
    // 客户端超时是长轮询的正常控制流，降为 debug，避免刷屏。
    if (classified.type === "timeout") logger.debug(message);
 ...
 * after the host has already aborted the long-poll.
 */
